...
fs.readFile("in.txt", "utf8", (err, data) => {
  ...
  const grid: Array<Array<MazeNode>> = lines.map((line, y) =>
  ...
  const path = aStar(entryNode, exitNode, grid);
  if (path === undefined) {
    console.log("NO PATH");
  } else {
    const visualPath = visualizePath(path, grid);
    console.log(visualPath);
    console.log("Nodes in path: ", path.length);
